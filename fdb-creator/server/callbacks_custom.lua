...
Callback.register('murphy_creator:GetPedData', function(source, charid)
    local citizenid = charid or GetCharIdentifier(source)
    if not citizenid then return {} end
    
    local result = MySQL.Sync.fetchAll('SELECT peddata FROM murphy_creator WHERE charid = ?', { citizenid })
...
Callback.register('murphy_creator:UpdatePedData', function(source, CachePedData)
    local citizenid = GetCharIdentifier(source)
    if not citizenid then return false end
    
    local result = MySQL.Sync.fetchAll('SELECT id FROM murphy_creator WHERE charid = ?', { citizenid })
    if result[1] then
        MySQL.Async.execute('UPDATE murphy_creator SET peddata = ? WHERE charid = ?', { json.encode(CachePedData), citizenid })
    else
        local info = { lore = "", birthday = "", birthmonth = "", birthyear = "" }
        MySQL.Async.execute('INSERT INTO murphy_creator (charid, peddata, informations) VALUES (?, ?, ?)', { citizenid, json.encode(CachePedData), json.encode(info) })
    end
    return true
end)

-- Callback CreateNewCharacter: Handles character creation and links features
Callback.register('murphy_creator:CreateNewCharacter', function(source, data, CachePedData)
    local src = source
    ...
    if citizenid then
        ...
        local result = MySQL.Sync.fetchAll('SELECT id FROM murphy_creator WHERE charid = ?', { citizenid })
        if result[1] then
            MySQL.Async.execute('UPDATE murphy_creator SET peddata = ?, informations = ? WHERE charid = ?', { json.encode(CachePedData), json.encode(info), citizenid })
        else
            MySQL.Async.execute('INSERT INTO murphy_creator (charid, peddata, informations) VALUES (?, ?, ?)', { citizenid, json.encode(CachePedData), json.encode(info) })
        end
        return true
...
Callback.register('murphy_creator:SavePreset', function(source, hairstyleCache, overlay_all_layers, name, outfitId, isMale)
    local citizenid = GetCharIdentifier(source)
    ...
    local result = MySQL.Sync.fetchAll('SELECT charid FROM murphy_barber WHERE charid = ?', { citizenid })
    if result[1] then
        MySQL.Async.execute('UPDATE murphy_barber SET hairstyle = ?, overlays = ?, permanentoverlay = ?, outfit_id = ? WHERE charid = ?', 
            { json.encode(hairstyleCache), json.encode(makeup), json.encode(permanent), outfitId or 0, citizenid })
    else
        MySQL.Async.execute('INSERT INTO murphy_barber (charid, hairstyle, overlays, permanentoverlay, outfit_id) VALUES (?, ?, ?, ?, ?)', 
            { citizenid, json.encode(hairstyleCache), json.encode(makeup), json.encode(permanent), outfitId or 0 })
    end
    
    local price = 0.0
    local gender = isMale and "Male" or "Female"
    MySQL.Async.execute('INSERT INTO murphy_barber_preset (charid, outfit_id, price, name, hairstyle, overlays, gender) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
Callback.register('murphy_barber_creator:GetCurrentHairs', function(source)
    local citizenid = GetCharIdentifier(source)
    if not citizenid then return {}, 0, {}, {} end
    
    local result = MySQL.Sync.fetchAll('SELECT * FROM murphy_barber WHERE charid = ?', { citizenid })
    if result[1] then
...
Callback.register('murphy_barber_creator:GetCurrentHairsOnCharacter', function(source, charid)
    local citizenid = charid or GetCharIdentifier(source)
    if not citizenid then return {}, 0, {}, {} end
...
Callback.register('murphy_barber_creator:GetCurrentOverlays', function(source)
    local citizenid = GetCharIdentifier(source)
    if not citizenid then return {}, {} end
...
-- Server Event murphy_barber_creator:updatehairstyle
RegisterServerEvent("murphy_barber_creator:updatehairstyle")
AddEventHandler("murphy_barber_creator:updatehairstyle", function(barberCache, barber_overlay_all_layers, outfitid)
    local src = source
