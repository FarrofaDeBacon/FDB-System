...
RegisterNetEvent('fdb-inventory:server:initiateTrade', function(targetId)
	...
	SetTimeout(30000, function()
		...
		if req and req.targetId == targetId and req.requestId == requestId then
			...
			TriggerClientEvent('fdb-inventory:client:tradeRequestCancelled', src)
...
--RegisterNetEvent('fdb-inventory:server:addTradeItem', function(tradeId, item, amount)
lib.callback.register('fdb-inventory:server:addTradeItem', function(source, tradeId, item, amount)
    local src = source
...
lib.callback.register('fdb-inventory:server:removeTradeItem', function(source, tradeId, tradeSlot, targetSlot, amount)
    local src = source
    if isOnCooldown(src) then return false end
...
RegisterNetEvent('fdb-inventory:server:confirmTrade', function(tradeId)
    ...
    if success then
        ...
        TriggerClientEvent('fdb-inventory:client:updateTrade', trade.initiator, failedTradeData)
        TriggerClientEvent('fdb-inventory:client:updateTrade', trade.target, failedTradeData)
