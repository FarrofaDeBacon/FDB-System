...
//				isTradeInventory: false,
        ...
        validateToken(csrfToken) {
            return axios
                .post("https://fdb-core/validateCSRF", {
...
**/	
		...
		acceptTradeInvite() {
			...
			axios.post('https://fdb-inventory/AcceptTradeRequest', {
		...
		declineTradeInvite(silent = false) {
			...
			axios.post('https://fdb-inventory/DeclineTradeRequest', {
				initiatorId: this.tradeInviteId
			}).catch((error) => {
...
*/		
		...
		async handleDropForBuyItem(targetSlot) {
			...
			try {				
				...
				const isNormalMode = await axios.post("https://fdb-inventory/CheckPurchase", {
					item: item,
		...
		async handleDropFromTradeToPlayer(targetSlot) {
			...
			try {
				...
				const response = await axios.post("https://fdb-inventory/RemoveTradeItem", {
...
//        },
		...
		shopError(itemInSlot) {
			axios.post("https://fdb-inventory/ShowShopError", JSON.stringify({
				item: itemInSlot
			}), {
				headers: {
		...
		async attachNotepad(item) {
			...
			try {
				await axios.post("https://fdb-inventory/attachNotepad", {
					item: item,
					slot: item.slot
				});
			} catch (error) {
		...
		async renameNotepad(item) {
			...
			try {
				await axios.post("https://fdb-inventory/renameNotepad", {
					item: item,
					slot: item.slot
				});
			} catch (error) {
		...
		async postData(eventName, data = {}) {
			try {
				const response = await axios.post(`https://fdb-inventory/${eventName}`, data);
...
*/
        ...
        async addItemToTrade(item, amount) {
			...
			try {
				const response = await axios.post("https://fdb-inventory/AddTradeItem", {
					tradeId: this.tradeId,
					item: item,
					amount: amountToAdd,
				});
