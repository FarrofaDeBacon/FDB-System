...
function jo.framework.UserClass:get(source)
  ...
  local data = FDBCore.Functions.GetPlayer(self.source)
  if not data then
...
function jo.framework:registerUseItem(item, closeAfterUsed, callback)
  ...
  local itemExists = FDBCore.Shared.Items[item] ~= nil
  if not itemExists then
    return eprint(item .. " < item does not exist in the core configuration")
  end
  FDBCore.Functions.CreateUseableItem(item, function(source, data)
...
function jo.framework:createInventory(id, name, invConfig)
  ...
  -------------
  -- FDB Inventory is defined in g, not kg
  -------------
...
function jo.framework:createUser(source, data, spawnCoordinate, isDead)
  ...
  FDBCore.Player.CheckPlayerData(source, convertData)
  jo.triggerEvent.server(source, "fdb-appearance:server:SaveSkin", data.skin, data.comps)
end
...
-- New character event
RegisterNetEvent("fdb-multicharacter:server:createCharacter", function()
  local source = source
  pendingCharacterSelection[source] = true
end)

-- Existing character event
RegisterNetEvent("fdb-multicharacter:server:loadUserData", function()
  local source = source
