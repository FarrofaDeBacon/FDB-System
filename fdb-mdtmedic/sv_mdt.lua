...
RegisterCommand(""..Config.Command.."", function(source, args)
	...
	local tablex = {}
    local _source = source
	local xPlayer = FDBCore.Functions.GetPlayer(_source)
	local group = FDBCore.Functions.GetPermission(_source)
...
AddEventHandler("rsg-mdtmedic:getOffensesAndOfficer", function()
	...
	local xPlayer = FDBCore.Functions.GetPlayer(usource)
	...
	exports.oxmysql:query("SELECT * FROM `players` WHERE `charinfo` LIKE ?", {string.lower('%'..query..'%')}, function(result) -- % wildcard, needed to search for all alike results
		...
		for index, data in ipairs(result) do
			if data.charinfo then
				...
				local core = FDBCore.Functions.GetPlayerByCitizenId(data.citizenid)
