-- fdb-propeditor/client/freecam.lua
...
local placementCallbackData = nil
local placementResourceName = nil
...
local function drawPointMarker()
    ...
    if ghostEntity then
        local zOffset = 0.0
        if IsEntityAPed(ghostEntity) and IsPedHuman(ghostEntity) then
            zOffset = 1.0
        end
...
local function finish(ok)
    ...
    -- Enviar resultado de volta ao recurso requisitante
    if placementResourceName then
        TriggerEvent(placementResourceName .. ":placementFinished", ok, resultData, placementType, placementModel, placementCallbackData)
    end
end
