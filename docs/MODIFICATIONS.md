...
- **Melhorias e Ajustes Arquiteturais (Revisão Técnica)**:
  - **Handshake NUI Real**: Removido o delay fixo de `Wait(1000)` em `theme.lua` que introduzia riscos de corrida. Agora, o Svelte sinaliza ao client quando seu ciclo `onMount` é concluído disparando um POST para a rota `/nuiReady`, acionando `fdb.theme.sync()` no milissegundo exato do carregamento.
  - **Ponte com o FDB-Core (`GetPlayer`)**: O arquivo `bridge.lua` foi migrado de esqueleto vazio para uma lógica de framework real que obtém `FDBCore` via exports. Implementa de forma transparente `GetPlayer(source)` no servidor e `GetPlayerData()` no client, além de uma checagem de estado `IsLoggedIn()`.
