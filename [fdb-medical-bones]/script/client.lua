...
local damageThreshold = 100  -- Valor de dano necessÃ¡rio para quebrar ossos
...
AddEventHandler("myScript:emitLight", function()
    ...
    if object ~= 0 then
        ...
        -- Obtenha a posiÃ§Ã£o do objeto
        local objectPos = GetEntityCoords(object)

        -- Adicione um offset para mover a luz para a posiÃ§Ã£o desejada
        ...
        Citizen.CreateThread(function()
            while true do
                ...
                -- Pare de desenhar a luz apÃ³s 10 segundos
                Citizen.Wait(10000)
                break
            end
        end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(500) -- controla a frequÃªncia das verificaÃ§Ãµes
        local playerPed = PlayerPedId()
        local pos = GetEntityCoords(playerPed)
...
function GetBodyPartFromBone(bone)
  ...
  local ledger, pen -- VariÃ¡veis globais para guardar as referÃªncias aos objetos
  ...
  function playAnim(animDictMale, animDictFemale, animName, duration, isUpperBody)
      ...
      Citizen.SetTimeout(duration, function()
          ...
          if DoesEntityExist(ledger) then -- Verifique se o objeto existe antes de tentar deletÃ¡-lo
              DeleteObject(ledger)
          end
          if DoesEntityExist(pen) then -- Verifique se o objeto existe antes de tentar deletÃ¡-lo
              DeleteObject(pen)
          end
      end)
  end
...
AddEventHandler('Notification:left_doctor', function(t1, t2, dict, txtr, timer)
    if Config.framework == "redemrp" then
        ...
        if txtr ~= nil then
            exports.fdb-medical-bones.LeftNot(0, tostring(t1), tostring(t2), tostring(dict), tostring(txtr), tonumber(timer))
        else
            local txtr = "tick"
            exports.fdb-medical-bones.LeftNot(0, tostring(t1), tostring(t2), tostring(dict), tostring(txtr), tonumber(timer))
        end
...
AddEventHandler('bone:Tip', function(text, duration)
    exports.fdb-medical-bones.DisplayTip(0, tostring(text), tonumber(duration))
end)


-- CÃ³digo do cliente para executar a animaÃ§Ã£o
RegisterNetEvent('bandage:playAnim')
AddEventHandler('bandage:playAnim', function()
    local playerPed = PlayerPedId() -- ObtÃ©m o ID do ped/personagem do jogador
    -- local animDict = "mini_games@story@mob4@heal_jules@bandage@arthur" -- DicionÃ¡rio de animaÃ§Ã£o
    -- local animName = "bandage_start" -- Nome da animaÃ§Ã£o
    heal=true
    ...
    -- Carregar a animaÃ§Ã£o
    -- RequestAnimDict(animDict)
    -- while not HasAnimDictLoaded(animDict) do
    --     Citizen.Wait(10)
    -- end
    
    -- Faz o ped do jogador executar a animaÃ§Ã£o
...
AddEventHandler('bone:DeathTimer', function()
    ...
    Citizen.CreateThread(function()
        while deathSecondsRemaining > 0 do
            Wait(1000)
            -- TriggerEvent("bone:Tip", 'æ²»ç–—çŠ¶æ€'..heal, 1000)
            ...
            TriggerEvent("bone:Tip", 'åœ¨'..deathSecondsRemaining..'ç§’åŽæ­»äº¡..', 1000)
        end
