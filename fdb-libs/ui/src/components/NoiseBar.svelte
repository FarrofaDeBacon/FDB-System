<script>
    ...
    // Configurações padrão (podem ser sobrescritas no ShowNoiseBar)
    let config = {
        title: "Risco",
    ...
    function handleMessage(event) {
        if (event.data.action === 'showNoiseBar') {
            if (event.data.config) {
                config = { ...config, ...event.data.config };
            }
            visible = true;
            value = 0;
        } else if (event.data.action === 'updateNoiseBar') {
            value = Math.min(100, Math.max(0, event.data.value));
        } else if (event.data.action === 'hideNoiseBar') {
            visible = false;
        }
    }
