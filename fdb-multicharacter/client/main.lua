...
CreateThread(function()
    while true do
        ...
        if NetworkIsSessionStarted() then
            Wait(500)
            TriggerEvent('fdb-multicharacter:client:chooseChar')
            return
        end
    end
end)
...
local function openCharMenu(bool)
    FDBCore.Functions.TriggerCallback("fdb-multicharacter:server:GetNumberOfCharacters", function(result)
...
RegisterNetEvent('fdb-multicharacter:client:closeNUI', function()
    cleanPed(charPed)
    SetNuiFocus(false, false)
end)

RegisterNetEvent('fdb-multicharacter:client:chooseChar', function()
    selectingChar = true
...
RegisterNUICallback('cDataPed', function(data, cb) -- Visually seeing the char
    ...
    if cData ~= nil then
        if cached_player_skins[cData.citizenid] then
            ...
            FDBCore.Functions.TriggerCallback('fdb-multicharacter:server:getAppearance', function(appearance)
...
RegisterNUICallback('closeUI', function(data, cb)
    DoScreenFadeOut(10)
    openCharMenu(false)
    TriggerServerEvent('fdb-multicharacter:server:loadUserData', data)
    cleanPed(charPed)
    cb('ok')
end)
...
RegisterNUICallback('disconnectButton', function(data, cb)
    cleanPed(charPed)
    TriggerServerEvent('fdb-multicharacter:server:disconnect')
    cb('ok')
end)
...
RegisterNUICallback('selectCharacter', function(data, cb)
    ...
    if DataSkin ~= nil then
        DoScreenFadeOut(10)
        TriggerServerEvent('fdb-multicharacter:server:loadUserData', cData)
        openCharMenu(false)
        cleanPed(charPed)
        Wait(5000)
...
RegisterNUICallback('setupCharacters', function(data, cb) -- Present char info
    FDBCore.Functions.TriggerCallback("fdb-multicharacter:server:setupCharacters", function(result)
        cached_player_skins = {}
        SendNUIMessage({
...
RegisterNUICallback('removeCharacter', function(data, cb) -- Removing a char
    TriggerServerEvent('fdb-multicharacter:server:deleteCharacter', data.citizenid)
    Wait(200)
    FDBCore.Functions.TriggerCallback("fdb-multicharacter:server:setupCharacters", function(result)
...
CreateThread(function()
    if LocalPlayer.state['isLoggedIn'] then
        exports['fdb-core']:createPrompt('unstick', vector3(-549.77, -3778.38, 238.60), FDBCore.Shared.Keybinds['J'], 'Set Me Free!', {
            type = 'client',
            event = 'fdb-multicharacter:client:unstick',
        })
    end
end)

RegisterNetEvent('fdb-multicharacter:client:unstick', function()
